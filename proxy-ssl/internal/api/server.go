...
func NewServer(cfg *config.Config, certIssuer *issuer.Issuer, logger *logrus.Logger) *Server {
	server := &Server{
		config: cfg,
		issuer: certIssuer,
		logger: logger,
...
func (s *Server) createAutoCertificate(c *gin.Context) {
	...
	if err := s.validateAutoCertificateRequest(&req, nodeID); err != nil {
		s.logger.WithError(err).Error("Invalid auto certificate request")
	...
	if err != nil {
		s.logger.WithError(err).Error("Failed to generate private key")
	...
	if err != nil {
		s.logger.WithError(err).Error("Failed to generate CSR")
	...
	if err := os.MkdirAll(s.config.CertStoragePath, 0o700); err == nil {
		if werr := os.WriteFile(filepath.Join(s.config.CertStoragePath, order.ID+".key"), privateKey, 0o600); werr != nil {
			s.logger.WithError(werr).Warn("Failed to persist private key for renewal")
		}
	} else {
		s.logger.WithError(err).Warn("Failed to ensure cert storage path for key persistence")
	}

	// Wait for certificate to be issued
	certBundle, err := s.waitForCertificate(nodeID, order.ID)
	if err != nil {
		s.logger.WithError(err).Error("Failed to get certificate")
...
func (s *Server) createOrder(c *gin.Context) {
	...
	if err := s.validateCreateOrderRequest(&req); err != nil {
		s.logger.WithError(err).Error("Invalid create order request")
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (s *Server) getCertificateBundle(c *gin.Context) {
	...
	if err != nil {
		s.logger.WithError(err).Error("Failed to get certificate bundle")
...
func (s *Server) renewCertificate(c *gin.Context) {
	...
	if err != nil {
		s.logger.WithError(err).Error("Failed to renew certificate")
...
func (s *Server) generateToken(c *gin.Context) {
	...
	if err := s.validateGenerateTokenRequest(&req); err != nil {
		s.logger.WithError(err).Error("Invalid token generation request")
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (s *Server) waitForCertificate(nodeID, orderID string) ([]byte, error) {
	...
	for attempt <= maxAttempts {
		...
		case "processing", "pending":
			s.logger.WithField("order_id", orderID).Debugf("Certificate still being processed... (attempt %d/%d)", attempt, maxAttempts)
			time.Sleep(10 * time.Second)
		default:
			s.logger.WithField("order_id", orderID).Warnf("Unknown status: %s", order.Status)
			time.Sleep(10 * time.Second)
		}
