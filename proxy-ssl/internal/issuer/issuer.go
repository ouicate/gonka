...
func New(cfg *config.Config, logger *logrus.Logger) (*Issuer, error) {
...
func (i *Issuer) issueCertificate(orderID string, csrBytes []byte, fqdns []string) {
	...
	i.logger.WithField("order_id", orderID).Infof("Issuing certificate using %s provider", i.provider.GetName())

	// Issue certificate using the configured provider
	certBundle, err := i.provider.IssueCertificate(csrBytes, fqdns)
	if err != nil {
		i.logger.WithError(err).WithField("order_id", orderID).Error("Failed to issue certificate")
	...
	if err != nil {
		i.logger.WithError(err).WithField("order_id", orderID).Error("Failed to save certificate bundle")
	...
	if leaf == nil {
		i.logger.WithField("order_id", orderID).Error("No certificate found in PEM bundle")
		i.mu.Lock()
	...
	i.logger.WithField("order_id", orderID).Info("Certificate issued successfully")
}
...
func (i *Issuer) renewCertificate(orderID string) {
	...
	if !exists {
		i.logger.WithField("order_id", orderID).Error("Order not found for renewal")
		return
	}
	...
	if err != nil {
		i.logger.WithError(err).WithField("order_id", orderID).Error("Failed to read stored private key for renewal")
		return
	}
	...
	if err != nil {
		i.logger.WithError(err).WithField("order_id", orderID).Error("Failed to generate CSR for renewal")
		return
	}
	...
	if err != nil {
		i.logger.WithError(err).WithField("order_id", orderID).Error("Failed to renew certificate")
		return
	}
	...
	if err := os.WriteFile(bundlePath, certBundle, 0644); err != nil {
		i.logger.WithError(err).WithField("order_id", orderID).Error("Failed to save renewed certificate")
		return
	}
	...
	i.logger.WithField("order_id", orderID).Info("Certificate renewal completed")
}
