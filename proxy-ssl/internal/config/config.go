...
func Load() (*Config, error) {
	...
	viper.SetDefault("use_mock_provider", true) // Default to mock for safety
	...
	cfg := &Config{
		...
		AllowedSubdomains: strings.Split(viper.GetString("cert_issuer_allowed_subdomains"), ","),
		...
		UseMockProvider:   viper.GetBool("use_mock_provider"),
	}
...
func (c *Config) validateDNSProviderConfig() error {
	...
	case "cloudflare":
		...
		cfLegacyToken := c.DNSProviderConfig["CLOUDFLARE_API_TOKEN"]
		cfEmail := c.DNSProviderConfig["CLOUDFLARE_EMAIL"]
		cfAPIKey := c.DNSProviderConfig["CLOUDFLARE_API_KEY"]

		if (cfZone != "" && cfDNS != "") || (cfLegacyToken != "") || (cfEmail != "" && cfAPIKey != "") {
			// ok
		} else {
			return fmt.Errorf("Cloudflare credentials required: set CF_ZONE_API_TOKEN and CF_DNS_API_TOKEN (preferred), or CLOUDFLARE_API_TOKEN, or CLOUDFLARE_EMAIL + CLOUDFLARE_API_KEY")
		}
	case "gcloud":
