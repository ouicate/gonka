...
func TestSubmitSeed(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			inputMsg: &types.MsgSubmitSeed{
				Creator:    testutil.Creator,
				EpochIndex: 15, // Invalid epoch
				Signature:  "signature",
			},
			expectErr:    types.ErrEpochNotFound,
			expectCalled: false,
		},
		{
			name:                "effective epoch not found",
			effectiveEpochIndex: 0,
			inputMsg: &types.MsgSubmitSeed{
				Creator:    testutil.Creator,
				EpochIndex: 10,
				Signature:  "signature",
			},
			expectErr:    types.ErrEffectiveEpochNotFound,
			expectCalled: false,
		},
	}
