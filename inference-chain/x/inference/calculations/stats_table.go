...
// MissedStatTestLookup performs a deterministic, on-chain safe check using a pre-computed lookup table.
// It returns 'true' if the number of misses is acceptable (test passes).
// This uses hardcoded p0=0.10 and alpha=0.05.
func MissedStatTestLookup(nMissed, nTotal int) (bool, error) {
	if nTotal == 0 {
		return true, nil
	}
