...
func TestMissedStatTestLookupErrorConditions(t *testing.T) {
	tests := []struct {
		name     string
		nMissed  int
		nTotal   int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := MissedStatTestLookup(tt.nMissed, tt.nTotal)
			if (err != nil) != tt.wantErr {
				t.Errorf("MissedStatTestLookup() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.expected {
				t.Errorf("MissedStatTestLookup() = %v, want %v", got, tt.expected)
			}
		})
	}
}

func TestMissedStatTestLookup(t *testing.T) {
	tests := []struct {
		name     string
		nMissed  int
		nTotal   int
...
func TestMissedStatTestLookupSpecialCase990(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := MissedStatTestLookup(tt.nMissed, tt.nTotal)
			if err != nil {
				t.Errorf("MissedStatTestLookup() unexpected error = %v", err)
				return
			}
			if got != tt.expected {
				t.Errorf("MissedStatTestLookup() = %v, want %v", got, tt.expected)
...
func TestMissedStatTestLookupLookupMapBoundaries(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.description, func(t *testing.T) {
			...
			result, err := MissedStatTestLookup(0, tc.total)
			if err != nil {
...
func TestMissedStatTestLookupEdgeCasesSmallValues(t *testing.T) {
	...
	for _, total := range testTotals {
		t.Run(fmt.Sprintf("consistency_test_total_%d", total), func(t *testing.T) {
			...
			for missed := 0; missed <= total; missed++ {
				result, err := MissedStatTestLookup(missed, total)
				if err != nil {
				...
				if result {
					...
					for higherMissed := missed + 1; higherMissed <= total && higherMissed <= missed+5; higherMissed++ {
						higherResult, err := MissedStatTestLookup(higherMissed, total)
						if err != nil {
...
func BenchmarkMissedStatTestLookup(b *testing.B) {
	...
	for _, tc := range testCases {
		b.Run(tc.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				_, _ = MissedStatTestLookup(tc.nMissed, tc.nTotal)
			}
		})
	}
}

func BenchmarkMissedStatTestLookupWorstCase(b *testing.B) {
	// Benchmark the worst case scenario - searching through the entire table
	b.Run("worst_case_search", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_, _ = MissedStatTestLookup(115, 989) // Just below 990, uses table lookup
		}
	})
}
