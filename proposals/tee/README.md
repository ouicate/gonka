...
This document describes the concept of integrating Trusted Execution Environment (TEE) into Gonka.

## Problem

User request data is not protected from hosts/miners. Even when payloads are moved off-chain, operator can access the data by using custom binaries or modifying the execution environment.

## Proposal

Use Trusted Execution Environment (TEE) technology to isolate the full VM from the hypervisor:
...
Q1: Should the certificate be validated on-chain?
...
Q: Should full certificate validation be performed on-chain? Probably yes, on recording.
...
Q: How to provide redundancy when a Confidential MLNode becomes unavailable?
...
Q: Since Confidential MLNodes have less validation overhead, should they receive higher bitcoin-style rewards to incentivize enabling TEE?